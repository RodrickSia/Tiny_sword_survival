...
class Game:
    ...
    def draw(self):
        self.screen.fill((64, 128, 64))  # Green background
        
        # Draw map layers
        if self.map_loader:
            # Draw static map
            self.map_loader.draw_static_layers(self.screen)
            # Draw animated tiles
            self.map_loader.draw_animated_tiles(self.screen)
        else:
            # If no map, show error message
